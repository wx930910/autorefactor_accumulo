...
public class TransformingIteratorTest {
	...
	private void setUpTransformIterator(Class<? extends TransformingIterator> clazz, boolean setupAuths)
			throws IOException {
	...
	public void testCreatingIllegalVisbility() throws Exception {
		// illegal visibility created by transform should be filtered on scan, even if
		// evaluation is
		// done
	...
	public void testRangeStart() throws Exception {
		...
		checkExpected(new Range(new Key("row1", "cf2", "cq2", "vis1"), true, new Key("row1", "cf2", "cq3"), false),
				expected);
	}
	...
	public void testRangeEnd() throws Exception {
		...
		checkExpected(new Range(new Key("row1", "cf2", "cq2"), true, new Key("row1", "cf2", "cq2", "vis2"), false),
				expected);
	}
	...
	private void checkExpected(Range range, Set<ByteSequence> families, TreeMap<Key, Value> expectedEntries)
			throws IOException {
	...
	private static void putExpected(SortedMap<Key, Value> expected, int rowID, int cfID, int cqID, int cvID,
			PartialKey part) {
		String row = "row" + rowID;
		String cf = "cf" + cfID;
		String cq = "cq" + cqID;
	...
	public static class ColFamReversingCompactionKeyTransformingIterator
		...
		public void init(SortedKeyValueIterator<Key, Value> source, Map<String, String> options,
			...
			env = TransformingIteratorTest.mockIteratorEnvironment1();
	...
	public static class IllegalVisKeyTransformingIterator extends TransformingIterator {
		...
		protected void transformRange(SortedKeyValueIterator<Key, Value> input, KVBuffer output) throws IOException {
			while (input.hasTop()) {
				Key originalKey = input.getTopKey();
				output.append(new Key(originalKey.getRow(), originalKey.getColumnFamily(),
