...
public class InMemoryMapTest {
	...
	static public IteratorEnvironment mockIteratorEnvironment1() {
		SamplerConfiguration[] mockFieldVariableSampleConfig = new SamplerConfiguration[1];
		IteratorEnvironment mockInstance = Mockito.spy(IteratorEnvironment.class);
		mockFieldVariableSampleConfig[0] = null;
		try {
			Mockito.doAnswer((stubInvo) -> {
				return mockFieldVariableSampleConfig[0] != null;
			}).when(mockInstance).isSamplingEnabled();
			Mockito.doAnswer((stubInvo) -> {
				return mockFieldVariableSampleConfig[0];
	...
	static public IteratorEnvironment mockIteratorEnvironment2(SamplerConfigurationImpl sampleConfig) {
		SamplerConfiguration[] mockFieldVariableSampleConfig = new SamplerConfiguration[1];
		IteratorEnvironment mockInstance = Mockito.spy(IteratorEnvironment.class);
		mockFieldVariableSampleConfig[0] = sampleConfig.toSamplerConfiguration();
		try {
			Mockito.doAnswer((stubInvo) -> {
				return mockFieldVariableSampleConfig[0] != null;
			}).when(mockInstance).isSamplingEnabled();
	...
	static void testAndCallNext(SortedKeyValueIterator<Key, Value> dc, String row, String column, int ts, String val)
			throws IOException {
	...
	static void assertEqualsNoNext(SortedKeyValueIterator<Key, Value> dc, String row, String column, int ts,
			String val) {
		assertTrue(dc.hasTop());
		assertEquals(newKey(row, column, ts), dc.getTopKey());
		assertEquals(new Value(val.getBytes()), dc.getTopValue());
	...
	public void test6() throws Exception {
		...
		SortedKeyValueIterator<Key, Value> dc = ski1.deepCopy(InMemoryMapTest.mockIteratorEnvironment1());
	...
	private void deepCopyAndDelete(int interleaving, boolean interrupt) throws Exception {
		// interleaving == 0 intentionally omitted, this runs the test w/o deleting in
		// mem map
	...
	public void testLocalityGroups() throws Exception {
		...
		SortedKeyValueIterator<Key, Value> dc1 = iter1.deepCopy(InMemoryMapTest.mockIteratorEnvironment1());
	...
	public void testSample() throws Exception {
		...
		for (ConfigurationCopy config : Arrays.asList(config1, config2)) {
			...
			SortedKeyValueIterator<Key, Value> iter0dc1 = iter0.deepCopy(InMemoryMapTest.mockIteratorEnvironment1());
			SortedKeyValueIterator<Key, Value> iter0dc2 = iter0
					.deepCopy(InMemoryMapTest.mockIteratorEnvironment2(sampleConfig));
			SortedKeyValueIterator<Key, Value> iter1dc1 = iter1.deepCopy(InMemoryMapTest.mockIteratorEnvironment1());
			SortedKeyValueIterator<Key, Value> iter1dc2 = iter1
					.deepCopy(InMemoryMapTest.mockIteratorEnvironment2(sampleConfig));
			SortedKeyValueIterator<Key, Value> iter2dc1 = iter2.deepCopy(InMemoryMapTest.mockIteratorEnvironment1());
			SortedKeyValueIterator<Key, Value> iter2dc2 = iter2
					.deepCopy(InMemoryMapTest.mockIteratorEnvironment2(sampleConfig));
			...
			SortedKeyValueIterator<Key, Value> iter0dc3 = iter0.deepCopy(InMemoryMapTest.mockIteratorEnvironment1());
			SortedKeyValueIterator<Key, Value> iter0dc4 = iter0
					.deepCopy(InMemoryMapTest.mockIteratorEnvironment2(sampleConfig));
			SortedKeyValueIterator<Key, Value> iter1dc3 = iter1.deepCopy(InMemoryMapTest.mockIteratorEnvironment1());
			SortedKeyValueIterator<Key, Value> iter1dc4 = iter1
					.deepCopy(InMemoryMapTest.mockIteratorEnvironment2(sampleConfig));
			SortedKeyValueIterator<Key, Value> iter2dc3 = iter2.deepCopy(InMemoryMapTest.mockIteratorEnvironment1());
			SortedKeyValueIterator<Key, Value> iter2dc4 = iter2
					.deepCopy(InMemoryMapTest.mockIteratorEnvironment2(sampleConfig));
	...
	private void runInterruptSampleTest(boolean deepCopy, boolean delete, boolean dcAfterDelete) throws Exception {
		SamplerConfigurationImpl sampleConfig1 = new SamplerConfigurationImpl(RowSampler.class.getName(),
				ImmutableMap.of("hasher", "murmur3_32", "modulus", "2"));
		...
		if (deepCopy) {
			iter = iter.deepCopy(InMemoryMapTest.mockIteratorEnvironment2(sampleConfig1));
		}
	...
	public void testDifferentSampleConfig() throws Exception {
		...
		SamplerConfigurationImpl sampleConfig2 = new SamplerConfigurationImpl(RowSampler.class.getName(),
				ImmutableMap.of("hasher", "murmur3_32", "modulus", "9"));
	...
	public void testEmptyNoSampleConfig() throws Exception {
		...
		// when in mem map is empty should be able to get sample iterator with any
		// sample config
		MemoryIterator iter = imm.skvIterator(sampleConfig2);
		iter.seek(new Range(), LocalityGroupUtil.EMPTY_CF_SET, false);
	...
	public void testDeferredSamplerCreation() throws Exception {
		...
		SortedKeyValueIterator<Key, Value> dc = iter.deepCopy(InMemoryMapTest.mockIteratorEnvironment2(sampleConfig2));
		dc.seek(new Range(), LocalityGroupUtil.EMPTY_CF_SET, false);
