...
public class TableLoadBalancerTest {
	...
	public org.apache.accumulo.server.master.balancer.TableLoadBalancer mockTableLoadBalancer1() {
		org.apache.accumulo.server.master.balancer.TableLoadBalancer mockInstance = Mockito
				.spy(org.apache.accumulo.server.master.balancer.TableLoadBalancer.class);
		try {
			Mockito.doAnswer((stubInvo) -> {
				...
				replay(tops);
				return tops;
			}).when(mockInstance).getTableOperations();
			Mockito.doAnswer((stubInvo) -> {
				TServerInstance tserver = stubInvo.getArgument(0);
				TableId tableId = stubInvo.getArgument(1);
				return generateFakeTablets(tserver, tableId);
			}).when(mockInstance).getOnlineTabletsForTable(Mockito.any(), Mockito.any());
			Mockito.doAnswer((stubInvo) -> {
				return DefaultLoadBalancer.class.getName();
			}).when(mockInstance).getLoadBalancerClassNameForTable(Mockito.any());
	...
	private ServerContext createMockContext() {
		...
		final String instanceId = UUID.nameUUIDFromBytes(new byte[] { 1, 2, 3, 4, 5, 6, 7, 8, 9, 0 }).toString();
	...
	public void test() {
		...
		replay(context);
		ServerConfigurationFactory confFactory = new ServerConfigurationFactory(context, new SiteConfiguration()) {
			@Override
			public TableConfiguration getTableConfiguration(TableId tableId) {
				...
				return new TableConfiguration(context, tableId, dummyConf) {
					...
					public String get(Property property) {
						// fake the get table configuration so the test doesn't try to look in zookeeper
						// for
		...
		org.apache.accumulo.server.master.balancer.TableLoadBalancer tls = mockTableLoadBalancer1();
		tls.init(context2);
		tls.balance(state, migrations, migrationsOut);
		assertEquals(0, migrationsOut.size());

		state.put(mkts("10.0.0.2", "0x02030405"), status());
		tls = mockTableLoadBalancer1();
		tls.init(context2);
		tls.balance(state, migrations, migrationsOut);
		int count = 0;
