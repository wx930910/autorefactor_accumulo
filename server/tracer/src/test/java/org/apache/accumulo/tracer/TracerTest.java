...
public class TracerTest {
	static public Iface mockIface1() {
		Iface mockInstance = Mockito.spy(TestService.Iface.class);
		try {
			...
			}).when(mockInstance).checkTrace(Mockito.any(), Mockito.any());
	...
	static class SpanStruct {
		public SpanStruct(long traceId, long spanId, List<Long> parentIds, long start, long stop, String description,
	...
	@SuppressFBWarnings(value = { "UNENCRYPTED_SOCKET",
			"UNENCRYPTED_SERVER_SOCKET" }, justification = "insecure, known risk, test socket")
	@Test
	public void testThrift() throws Exception {
		...
		args.processor(new Processor<Iface>(TraceUtil.wrapService(TracerTest.mockIface1())));
	...
	/**
	 * Verify that exceptions propagate up through the trace wrapping with sampling
	 * enabled, as the cause of the reflexive exceptions.
	 */
	...
	/**
	 * Verify that exceptions propagate up through the trace wrapping with sampling
	 * disabled, as the cause of the reflexive exceptions.
	 */
	@Test(expected = IOException.class)
